  ...
  const { isPending, error, data } = useQuery({
    ...
    queryFn: async () => {
      const res = await axios.get(
        `${baseUrlChart}HealthStatus?userHealth=${params}`
      );
      return res.data;
    },
  });
  ...
  const mutation = useMutation({
    mutationFn: async (data) => {
      ...
      let temp = await axios.get(
        `http://localhost:8080/test/dashboard/AppHealth?userName=${data.UserName}`
      );
      return temp.data;
    },
...
export default ActiveDeviceSubPage
