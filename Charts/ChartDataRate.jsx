...
const ChartClientDataRate = () => {
  const [labels, setLabels] = useState([]);
  const classes = useStyles();

  const { isPending, error, data } = useQuery({
    queryKey: ["chartdataRate"],
    queryFn: async () => {
      const res = await axios.get(
        `${baseUrlChart}HealthOn?type=datarate`
      );
    ...
    <div style={{ padding: "10px", position: "relative" }}>
      ...
      <Card className={classes.root}>
        <CardContent className={classes.inside}>
          <Grid container spacing={1}>
            <Grid item xs={12}>
              <Typography>Client Data Rate</Typography>
            </Grid>
          </Grid>
        </CardContent>
      </Card>
...
export default React.memo(ChartClientDataRate);
